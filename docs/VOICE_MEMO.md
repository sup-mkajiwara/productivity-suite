...
### パターンB: ボイスメモ本体を直接監視（書き出し操作が不要）

**「共有 → ファイルに保存」の操作なしで**、録音するだけで自動処理されます。
...
監視用アプリ `~/.claude/state/VoiceMemoWatcher.app` がビルドされ、
launchd がそれを経由して起動するよう登録し直されます。

#### 3. フルディスクアクセスを許可する（**この操作が必須です**）

ボイスメモの録音は `~/Library/Group Containers/...` にあり、読むには
フルディスクアクセスが必要です。

1. システム設定 → プライバシーとセキュリティ → **フルディスクアクセス**
2. 「+」を押して次を追加し、スイッチをオンにする
   ```
   ~/.claude/state/VoiceMemoWatcher.app
   ```
   ※ Finder のファイル選択画面で **⌘⇧G** を押し、上のパスを貼り付けると選べます

> 💡 **なぜ専用アプリを経由するのか**
> フルディスクアクセスの許可はアプリバンドルに紐づくため、launchd が
> シェルスクリプトを直接起動しても許可を与えられません。専用アプリを
> 経由することで、**このアプリだけに権限を限定**できます
> （`/bin/zsh` に許可を与えると、あらゆるシェルスクリプトが全ファイルに
> アクセスできてしまいます）。
> 許可されたアプリから起動された子プロセスは、その権限を引き継ぎます。

#### 4. 動作を確認する

```bash
open -n -a ~/.claude/state/VoiceMemoWatcher.app
sleep 10 && tail -20 ~/.claude/logs/voice-memo-notes.log
```

「取り込み元フォルダがありません」と出る場合は、許可がまだ有効になっていません。
...
> 録音直後は書き込み途中のことがあるため、ファイルサイズが安定するまで待ってから処理します。
> 取りこぼしに備え、フォルダ監視に加えて10分ごとの定期チェックも行います。
